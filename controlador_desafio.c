/*
 * File:          controlador_desafio.c
 * Date:12/03/2021
 * Description:Sistema autonomo afim de guiar um robo ao objetivo final.
 * Author:Manoel Paulo da Silva Neto
 * Modifications:
 */
    ...
    switch (state) {
      ...
      case FORWARD:
        if (wheel_weight_total[0] > WHEEL_WEIGHT_THRESHOLD) {
          speed[0] = -2.34 * MAX_SPEED;
          speed[1] = 2.34 * MAX_SPEED;
          state = LEFT;
